...
- model
  ...
  - Number : 로또 번호를 관리하고 로또를 구성하는데 사용되는 핵심 모델.
  - Lottery, Lotteries : 로또 번호만 포함하는 일급 컬렉션.
  - Rank : 당첨 순위와 상금을 관리하는 열거형 모델. 당첨 기준에 해당하는 결과를 반환함
  - Winning : 당첨 번호와 보너스 번호를 저장하는 모델. 당첨 결과를 토대로 통계 자료를 생성함
  - Summary : 당첨 순위와 개수를 저장하는 모델. 세부 당첨 내역과 총 당첨 금액을 알수 있음
- service
